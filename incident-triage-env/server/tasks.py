...
TASKS: Dict[str, TaskDefinition] = {
    ...
    "task_3_resolve": TaskDefinition(
        ...
        grading_criteria={
            ...
            "root_cause_accuracy": 0.20,
            "remediation_accuracy": 0.25,
...
def grade_task_3(state: Any, incident: Any) -> float:
    ...
    if state.remediation_applied:
        if state.remediation_value and state.remediation_value.lower() == incident.correct_remediation.lower():
            ...
            # Partial credit for reasonable but suboptimal remediation
    ...
    if state.communicated:
        score += 0.10  # Base credit for communicating
        # Bonus for mentioning key info in actions
    ...
    if state.resolved:
        score += 0.10  # Base credit for resolving
        # Bonus for mentioning root cause and remediation in resolution
        action_text = " ".join(state.actions_taken).lower()
