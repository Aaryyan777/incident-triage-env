...
def run_heuristic_episode(task_id: str, incident_index: int, seed: int) -> Dict:
    ...
    if task_id in ("task_2_diagnose", "task_3_resolve"):
        ...
        env.step({"action_type": "diagnose", "root_cause": root_cause, "explanation": explanation})

    if task_id == "task_3_resolve":
        remediation = _heuristic_remediate(root_cause)
        env.step({"action_type": "remediate", "remediation": remediation})
        env.step({
...
def main():
    ...
    print(f"HF_TOKEN:     {'***' + HF_TOKEN[-4:] if len(HF_TOKEN) > 4 else '(not set)'}")
