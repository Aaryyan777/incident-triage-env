...
def run_llm_episode(
    ...
    while not env.is_done and step_count < task.max_steps:
        try:
            ...
            obs_dict = asdict(obs)
            step_count += 1
            print(f"[STEP] step={step_count} reward={obs_dict['reward']:.4f}", flush=True)
...
def run_heuristic_episode(task_id: str, incident_index: int, seed: int) -> Dict:
    ...
    def _step(action: dict):
        ...
        obs = env.step(action)
        obs_dict = asdict(obs)
        print(f"[STEP] step={env.state.step_count} reward={obs_dict['reward']:.4f}", flush=True)
...
def main():
    ...
    for task_id in TASKS:
        ...
        for i in range(NUM_INCIDENTS):
            ...
            print(f"[END] task={task_id} score={result['score']:.4f} steps={result['steps']}", flush=True)
