...
class IncidentTriageEnvironment:
    ...
    def step(
        ...
        **kwargs: Any,
        ...
        elif action_type == "request_specialist":
            reward, feedback = self._handle_request_specialist(action)
        elif action_type == "handoff":
            reward, feedback = self._handle_handoff(action)
        else:
            feedback = f"Action '{action_type}' not available for task '{self._task_id}'."
            reward = -0.05

        # ── Check cascade events ──────────────────────────────────
        cascade_feedback = self._check_cascades()
        if cascade_feedback:
            feedback += f" | {cascade_feedback}"
            reward -= 0.10  # Penalty per cascade
    ...
    def _handle_resolve(self, action: Dict[str, Any]) -> tuple:
        ...
        self._state.resolution_summary = str(summary)
        return reward, feedback

    def _check_cascades(self) -> str:
        """Check and fire cascade events based on current step and agent state."""
        if self._incident is None:
            return ""

        fired = []
        for ce in self._incident.cascade_events:
            ce_id = f"{ce.trigger_step}_{ce.condition}"
            if ce_id in self._state.cascades_fired:
                continue
            if self._state.step_count < ce.trigger_step:
                continue

            # Check condition
            should_fire = False
            if ce.condition == "not_remediated" and not self._state.remediation_applied:
                should_fire = True
            elif ce.condition == "not_classified" and not self._state.severity_classified:
                should_fire = True
            elif ce.condition == "not_diagnosed" and not self._state.diagnosed:
                should_fire = True

            if should_fire:
                # Apply cascade effects to the incident
                self._incident.symptoms.extend(ce.new_symptoms)
                self._incident.logs.extend(ce.new_logs)
                for k, v in ce.metric_changes.items():
                    self._incident.metrics[k] = v
                self._incident.affected_users += ce.affected_users_delta
                if ce.severity_escalation:
                    self._incident.severity = ce.severity_escalation

                self._state.cascade_count += 1
                self._state.cascades_fired.append(ce_id)
                fired.append(ce.feedback)

        return " ".join(fired)

    def _handle_request_specialist(self, action: Dict[str, Any]) -> tuple:
        """Handle specialist consultation request."""
        team = action.get("team", "") if isinstance(action, dict) else getattr(action, "team", "")
        team = str(team).lower().strip()

        valid_teams = ["platform", "database", "networking", "security", "application", "infrastructure"]
        if team not in valid_teams:
            return -0.05, f"Invalid specialist team: '{team}'. Must be one of {valid_teams}."

        self._state.specialist_consulted = True
        self._state.specialist_team = team

        # Get specialist response from incident data
        responses = getattr(self._incident, 'specialist_responses', {})
        response = responses.get(team, f"The {team} team has reviewed the incident. No specific insights available.")
        self._state.specialist_response = response

        # Bonus if consulting the correct team
        correct_team = self._incident.correct_team.lower()
        if team == correct_team:
            self._state.coordination_score = 1.0
            return 0.10, f"Specialist response from {team}: {response}"
        else:
            self._state.coordination_score = 0.3
            return 0.03, f"Specialist response from {team}: {response}"

    def _handle_handoff(self, action: Dict[str, Any]) -> tuple:
        """Handle incident handoff to another team."""
        team = action.get("team", "") if isinstance(action, dict) else getattr(action, "team", "")
        reason = action.get("reason", "") if isinstance(action, dict) else getattr(action, "reason", "")
        team = str(team).lower().strip()
        ...
        valid_teams = ["platform", "database", "networking", "security", "application", "infrastructure"]
        if team not in valid_teams:
            return -0.05, f"Invalid handoff team: '{team}'. Must be one of {valid_teams}."

        self._state.handoff_team = team

        correct_team = self._incident.correct_team.lower()
        if team == correct_team:
            reward = 0.15
            feedback = f"Incident handed off to {team} team. Correct team for this issue."
        else:
            reward = -0.10
            feedback = f"Incident handed off to {team} team. This may not be the best team for this issue."

        if reason and len(reason) > 10:
            reward += 0.03
            feedback += " Good handoff justification."
